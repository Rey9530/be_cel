...
export class ContractsService {
  ...
  async create(createProjectDto: CreateProjectDto, user: mar_usr_usuario) {
    let ctr_fecha_inicio = convert_date(createProjectDto.ctr_fecha_inicio);
    let ctr_fecha_fin = convert_date(createProjectDto.ctr_fecha_fin);
  ...
  async addEmployeToContract(
    ...
    let contract = await this.prisma.mar_ctr_contratos.findFirst({
      where: {
        ctr_codusr: user.usr_codigo,
        ctr_codigo: idCtr,
        ctr_estado: 'ACTIVE',
    ...
    let horario = await this.prisma.mar_hor_horarios.findFirst({
    ...
    let asignacion = await this.prisma.mar_asi_asignacion.create({
      data,
    });
  ...
  async createSchedule(
    ...
    //HACER: Terminar de agregar validaciones
    try {
      let contract = await this.prisma.mar_ctr_contratos.findFirst({
        where: {
          ctr_codusr: user.usr_codigo,
      ...
      let horarios = await this.prisma.mar_hor_horarios.count({
        where: { hor_codctro: contract.ctr_codigo, hor_estado: 'ACTIVE' },
      });
      let usrName = user.usr_nombres + ' ' + user.usr_apellidos;
      let newHours = await this.prisma.mar_hor_horarios.create({
        data: {
      ...
      let saveDb = await this.prisma.mar_hde_detalle_ho.createMany({
        data: hours,
      });
      return saveDb;
    } catch (error) {
  ...
  async updateSchedules(
    ...
    //HACER: TERMINAR ESTOOO
    try {
      let respDb = await this.prisma.mar_hor_horarios.findFirst({
        where: {
          hor_codigo: id,
          hor_estado: 'ACTIVE',
        },
      ...
      let usrName = user.usr_nombres + ' ' + user.usr_apellidos;
      let resp = []; 
        for (const item of updateScheduleDto.mar_hde_detalle_ho) {
        ...
        let respDB = await this.prisma.mar_hde_detalle_ho.update({
  ...
  getHour(element) {
    ...
    let dateResp;
    if (hdefin2.length == 7) {
      let h = Number(`${hdefin2[0] + hdefin2[1]}`);
      if (hdefin2[5] == 'P') {
        h = h < 12 ? h + 12 : h;
      }
  ...
  async findOne(id: string) {
    let respDb;
    try {
      respDb = await this.prisma.mar_ctr_contratos.findFirst({
  ...
  async deleteEmployes(idAsi: string, user: mar_usr_usuario) {
    let data = await this.prisma.mar_asi_asignacion.findFirst({
  ...
  async deleteSchedule(id: string, user: mar_usr_usuario) {
    let data = await this.prisma.mar_hor_horarios.findFirst({
  ...
  async listEmployes(idCtr: string, user: mar_usr_usuario) {
    let contracts = await this.prisma.mar_ctr_contratos.findFirst({
      where: {
        ctr_estado: 'ACTIVE',
        ctr_codigo: idCtr,
        ctr_codusr: user.usr_codigo,
    ...
    let empleados = await this.prisma.mar_hor_horarios.findMany({
      where: {
        hor_codctro: idCtr,
        hor_estado: 'ACTIVE',
      },
  ...
  async getEmployes(
    ...
    let empresa = await this.prisma.mar_ctr_contratos.findFirst({
      where: {
        ctr_estado: 'ACTIVE',
        ctr_codigo: id,
        ctr_codusr: user.usr_codigo,
  ...
  async update(
    ...
    let ctr_fecha_inicio = convert_date(updateProjectDto.ctr_fecha_inicio);
    let ctr_fecha_fin = convert_date(updateProjectDto.ctr_fecha_fin);
    ctr_fecha_fin.setHours(ctr_fecha_fin.getHours() + 23);
    ctr_fecha_fin.setMinutes(ctr_fecha_fin.getMinutes() + 59);
    if (ctr_fecha_inicio.getTime() > ctr_fecha_fin.getTime()) {
      throw new InternalServerErrorException(
    ...
    try {
      ...
      let respDb = await this.prisma.mar_ctr_contratos.update({
        where: { ctr_codigo: id, ctr_estado: 'ACTIVE' },
        data,
      });
      if (!respDb) throw new NotFoundException('Registro no encontrado');
  ...
  async remove(id: string, user: mar_usr_usuario) {
    ...
    let respDb;
    try {
      respDb = await this.prisma.mar_ctr_contratos.update({
        where: { ctr_codigo: id, ctr_estado: 'ACTIVE' },
        data: {
          ctr_estado: 'INACTIVE',
          ctr_usrmod: user.usr_nombres + ' ' + user.usr_apellidos,
